...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  void initState() {
    ...
    _selectedStyleId = widget.userProfile['preferred_style_id']?.toString();
    _profilePicUrl = widget.userProfile['user_profile_pic']?.toString();

    final gender = widget.userProfile['user_gender']?.toString() ?? '';
  ...
  Future<void> _removeImage() async {
    if (_selectedImage != null) {
      setState(() {
        _selectedImage = null;
        _errorMessage = '';
        _successMessage = 'Selected image removed.';
      });
      return;
    }
    ...
    try {
      ...
      await _profileService.deleteProfilePicture(user.id);

      if (!mounted) return;
      setState(() {
        _profilePicUrl = null;
        _successMessage = 'Profile picture removed successfully!';
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
                  ...
                  child: DropdownButton<String>(
                    ...
                    items: _styles.map((style) {
                      return DropdownMenuItem<String>(
                        ...
                        child: Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 16),
                          child: Text(style['style_name']?.toString() ?? ''),
                        ),
                      );
                    }).toList(),
                    onChanged: (String? newValue) {
                      setState(() {
                        _selectedStyleId = newValue;
                      });
                    },
                    icon: const Padding(
                      padding: EdgeInsets.only(right: 16),
